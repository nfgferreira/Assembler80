...
char le_car(void)
{
    ...
    if (!resta_car) /* caso nao ha' mais caracteres na linha atual */
    {
        while (1) /* loop infinito: so' sai com break */
        {
            ...
            {
                paux0 = tab_end_chars[(mac_char >> n_rot_macro_char) & cmsk_rot_macro_char] + (mac_char & msk_rot_macro_char);
...
void final_de_linha(void)
{
    if (ppmac) /* caso expandindo macro */
    {
        while (*(tab_end_chars[(mac_char >> n_rot_macro_char) & cmsk_rot_macro_char] + (mac_char & msk_rot_macro_char)) != '\n')
            mac_char++;
        mac_char++;
    }
