    ...
    case DB:
        ...
        do
        {
            ...
            else
            {
                ...
                else if (!((pilha[0].atr & ABS) && (!(pilha[0].valor & ~0xff) || ((pilha[0].valor & ~0x7f) == ~0x7f))))
...
int inst_z80(void)
{
    ...
    return (inst = (*(tab_mem_aloc[(pmem >> bit_rot_mem) & ((1 << (16 - bit_rot_mem)) - 1)] + (pmem & masc_mem))) & 0xff) == 0xcb || inst == 0xdd || inst == 0xed || inst == 0xfd;
}
